...
class MainActivity : AppCompatActivity() {
    ...
    private val reminders = arrayListOf<Portal>()
    private val reminderAdapter = PortalAdapter(reminders)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
     ...
     /*       // Code to add to the floating button onClickListener:
            val name = etReminder.text.toString()
            addReminder(name)
*/
...
*/
        ...
        if (resultCode == Activity.RESULT_OK) {
            when (requestCode) {
                ADD_REMINDER_REQUEST_CODE -> {
                    val reminder = data!!.getParcelableExtra<Portal>(EXTRA_REMINDER)
    ...
    private fun initViews() {
        ...
        rvReminders.layoutManager = GridLayoutManager(this@MainActivity, 2)
